...
public class Enemy01 : Enemy
{
    ...
    {
        if (!usedTurn && firstTurn && (distance >= fieldOfVision - 0.75f) && (((target.position.x > transform.position.x) && transform.localScale.x > 0)
    ...
    void MoveToTarget()
    {
        float dir = transform.position.x > target.position.x ? -1f : 1f;
